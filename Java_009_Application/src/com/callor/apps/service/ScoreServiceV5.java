...
public class ScoreServiceV5 {
	...
	/*
	 * 현재는 학생수 만큼 모든 과목의 점수 입력을 끝내야만 다른 업무로 진행이 된다. 몇명의 학생, 일부과목 점수만 입력마치고 잠시 업무를
	 * 멈추고 싶을때 현재 코드는 불가능하다 => 일부만 입력하고 업무를 중단하는 기능을 넣자
	 */
	public void makeScores() {

		// 과목의 갯수만큼 점수를 담을 임시 배열 선언
		...
		for (int i = 0; i < intKor.length; i++) {
			...
			for (int j = 0; j < subject.length; j++) {
				 ...
				 * 이 때 break를 사용하면 for가 겹쳐있기 때문에
				 * 원하는대로 코드가 작동되지 않는다
				 * 이럴 경우 return type이 void 이지만
				 * return 명령을 실행하여 method 코드를 중단할 수 있다
				 */
				if(scores[j] == -1) {
					return;
				}
