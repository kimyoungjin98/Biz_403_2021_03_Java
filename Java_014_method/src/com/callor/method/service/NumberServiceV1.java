 ...
 * 	한개의 모듈(파일)또는 한 프로젝트서 같은 이름의 함수를 선언할 수 없다
 * 
 *  객체지향 프로그램언어(java에서는)
 ...
 *  단, 조건이 있는데
 *   	매개변수가 달라야 한다
 *  	매개변수의 type, 갯수, 순서가 다르면
 *  		같은 이름의 method를 중복선언 할수 있다.
 */
public class NumberServiceV1 {

	/*
	 * method 이름은 addNum() 두 개의 정수를 매개변수로 받는다 누구나, 어디선가 addNum()를 호출할때는 객체.addNum()
	 * 2개의 정수를 전달하게 하겠다 라는 선언 두 개의 정수를 전달받기 위해서는 method가 준비를 해야 한다 method의 () 안에 변수를
	 * 선언한다 두 개의 변수를 선언하자
	 * 
	 * 두 수의 합을 계산하여 결과가 짝수이면 그 합을 return return 합; 형식의 명령이 있어야 한다 그렇지 않으면 null을
	 * return return null; 형식의 명령이 있어야 한다
	 * 
	 * return 값 형식의 명령이 있으려면 void가 아닌 return type을 지정해 주어야 한다
	 * 
	 * 단순히 정수만 return을 하는 경우는 int type으로 하면되는데 여기에서는 어떤 조건에 따라 null값을 return 해야 하는
	 * 경우도 있기 때문에 int type이 아니고 Integer type으로 설정해야 한다
	 * 
	 */
	public Integer addNum(int num1, int num2) {
		// TODO 두 정수의 합이 짝수인가 판별
...
//		return sum;
	 ...
	 * 		addNum(float num1 , float num2) 형식으로 method를 선언해야 한다
	 * 
