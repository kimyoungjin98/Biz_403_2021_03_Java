...
public class IolistServiceV1 {
	...
	/*
	 * 매입 매출 데이터들을 사용하여 연산을 수행할텐데 매입매출 데이터들의 묶음을 List 형 객체 사용하겠다
	 * 
	 * List형 객체에 포함된 (개별)데이터는 IolistVO 형으로 사용하겠다
	 * 
	 * 낱개 데이터는 IolistVO 클래스 형으로 사용하고 그 데이터들을 묶어서 List 형으로 연산 수행을 하겠다 라는 선언
	 * 
	 * iolist를 private으로 선언한 이유 혹시 외부에서 iolist 데이터에 접근하여 값을 저장하는 일을 방지하기 위함이다 V1
	 * 클래스를 상속하여 사용할 예정이라면 protected로 선언을 한다
	 * 
	 */
	...
	// public으로 선언된 method는
	// 객체.method() 형식으로
	// 호출이 가능한 method
	// return type이 명확히 결정되지 않았을 때에는
	// void 형으로 작성을 한다
